...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync( string returnUrl = null )
        {
            ...
            {
                var uid = await CreateNewUser(Input.FirstName, Input.LastName, Input.DOB, Input.Department, Input.Role);
        ...
        private async Task<string> CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            ...
            try{
                ...
                _logger.LogError($"Error creating new user: {e.Message}");
                throw;
            }
            return uid;
        }
        ...
        private string GetUniqueUID(){
            ...
            string maxAdministratorUID = db.Administrators.Select(x => x.UId).DefaultIfEmpty().Max();
            string maxProfessorUID = db.Professors.Select(x => x.UId).DefaultIfEmpty().Max();
            string maxStudentUID = db.Students.Select(x => x.UId).DefaultIfEmpty().Max();

            // Find the maximum UID among the three tables
            if(maxAdministratorUID != null){
                maxUID = Math.Max(maxUID, int.Parse(maxAdministratorUID.Substring(1)));
            }

            if(maxProfessorUID != null){
                maxUID = Math.Max(maxUID, int.Parse(maxProfessorUID.Substring(1)));
            }

            if(maxStudentUID != null){
                maxUID = Math.Max(maxUID, int.Parse(maxStudentUID.Substring(1)));
            }
